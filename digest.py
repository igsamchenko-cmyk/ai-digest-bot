...
def gemini_call(client, contents, use_search=False, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            kwargs = {"model": os.environ.get("GEMINI_MODEL", "gemini-2.0-flash"), "contents": contents}
            if config:
...
def build_gemini_message(data, today_uk):
    ...
    for i, item in enumerate(data.get("news", []), 1):
        ...
        lines += [
            importance_icon.get(importance, "⚪") + " <b>" + str(i) + ". " + escape_text(item.get("title", "")) + "</b>",
            category_icon.get(category, "📌") + " <code>" + escape_text(category) + "</code>  //  " + escape_text(item.get("source", "")),
